...
def create_table():
    create_table_query = '''
    CREATE TABLE IF NOT EXISTS workshop_happiness(
...
def insert_data(row):
    insert_query = """
        INSERT INTO workshop_happiness (country, happiness_score, gdp_per_capita, social_support, freedom, life_expectancy, happiness_prediction)
    ...
    try:
        ...
        print('Error en la insercion: %s', error)
    finally:
        if cnx is not None:
            cnx.close()
